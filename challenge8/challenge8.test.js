...
const countNumberOfElements = (arr) => {
  return arr.reduce ((totalCount, value, i)=> {
    totalCount++;
    return totalCount;
  }, 0)
};
...
const returnNames = (arr) => {
  return arr.reduce((arrName, value) => {
...
const reversedString = (arr) => {
  let arrS = arr.split('');
  let result = arrS.reduce((reverse, value) => {
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((total, value) => {
    if(value.children){
      total = value.children.length + total;
    } 
    return total;
  },0);
};
...
const calculateAverage = (arr) => {
  ...
  let result = arr.reduce((accObj, value) => {
    accObj.sum = value + accObj.sum;
    accObj.count++;
    return accObj;
  }, {count:0, sum:0})
...
const countPrimeNumbers = (arr) => {
  ...
  let unprime = arr.reduce((notPrime, value) => {
  ...
  arr.filter(value => {
    if(!unprime.includes(value) && value>1){
      primeArr.push(value);
    }
    return primeArr;
  })
...
const extractChildren = (arr) => {
  let aArray = [];
  arr.filter(value => {
    if(/[a]/.test(value.name)){
      aArray.push(value);
    }
  })

  return aArray.reduce((kids, value) => {
    if (value.children){
      kids = kids.concat(value.children);
    }
    return kids;
  }, [])
};
