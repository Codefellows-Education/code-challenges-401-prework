...
const houseSize = (arr) => {
  ...
  arr.forEach(house => {
    ...
    if (house.children.length){
      totalMembers = house.children.length + totalMembers;
    }

    sizes.push({house:house.house, members:totalMembers});
  })
  return sizes;
}
...
/**
 * let originalArr = [{},{},...];
 * let formattedArr = originalArr.map(function(val){
 *  return {...};
 * });
 */
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(house => {
    ...
    if (!deceasedSpouses.includes(house.spouse)  && house.spouse !== null){
      totalMembers++;
    }
    if (house.children.length){
      totalMembers = house.children.length + totalMembers;
    ...
    survivors.push({house:house.house, members:totalMembers});
  })
  return survivors;
}
