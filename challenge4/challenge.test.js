...
const splitFoods = (recipe) => {
  ...
  for(let i=0; i<recipe.ingredients.length; i++){
    let recipeArr = recipe.ingredients[i].split(' ');
    result.push(recipeArr.slice(2).join(' '));
  }
  return result;
}
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach(step => {
    tempArr.push(step.split(' ').slice(0,1));
  })
...
const extractVowels = (str) => {
  ...
  arr.forEach(char => {
    if (vowels.includes(char)){
      ...
      constArray.push(char);
    }
  })
  finalArray.push(constArray.join(''));
  
  let sort = vowelArray.sort();
  finalArray.push(sort.join(''));
  return finalArray;
};
