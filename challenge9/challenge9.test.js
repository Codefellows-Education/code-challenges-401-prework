...
const count = (target, input) => {
  let total = 0;
  let newFinal = input.map(arr => {
    let finalCount = arr.reduce((count, value) => {
      if(value === target){
        count++;
      }
      return count;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  // let correctArr = [];
  // filter step
  // [ [0,2,5,4], [2,4,10], [] ] => [ [0,5], [10], [] ]
  let reformattedArr = input.map(arr => {
...
let findMaleAndFemale = (data) => {
  let obj = data.filter(personObj => {
    if(personObj.gender === 'male' || personObj.gender === 'female'){
      return personObj;
    }
  })
  let malePeople = obj.map (value => {
    return value.name;
    //console.log(male);
  })
  return malePeople.join(' and ');
}
...
let findShortest = (data) => {
  let shortest;
  data.forEach(person => {
    if(!shortest || Number(person.height) < Number(shortest.height)){
      shortest = person;
    }
  });
